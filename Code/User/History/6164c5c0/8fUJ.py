...
async def change_image(occupied: dict, schedule, path: Path):
    ...
    if occupied:
        # TODO списаться по поводу сортировки записей
        schedule.sort(key=lambda x: x['time'])
        now = datetime.now()
        schedule = [{'name': x['name'], 'time': datetime.strptime(x['time'], '%H:%M')} for x in schedule]
        schedule = [x for x in schedule if x['time'].replace(year=now.year, month=now.month, day=now.day) >= now]
        schedule = schedule[:3]
        # TODO максимально три записи с этого времени должны отображаться
        pass
    else:
