...
async def check_schedule():
    ...
    while True:
        ...
        if not response:
            page = await static_image(base_dir)
        else:
            try:
                occupied, schedule = parse_schedule(response)

                # await change_color(occupied, pixels)
                page = await change_image(occupied, schedule, base_dir)
            except:
                page = await static_image(base_dir)

        # TODO проверить: может быть проблема с открытием стиля

        html_bytes = page.encode("utf-8")
        html_b64 = base64.b64encode(html_bytes).decode("utf-8")

        webbrowser.register('vivaldi', None, webbrowser.BackgroundBrowser('C:/Program Files/Vivaldi/Application/vivaldi.exe'))
        webbrowser.open_new_tab("data:text/html;base64," + html_b64)
